...
- **类脑记忆** — 工作记忆 + 记忆整合 + 长期记忆（情景/语义/程序性），带遗忘与检索机制
- **计划模式** — Agent 可起草多步骤计划，经用户审批后逐步执行
- **主动式 Agent** — 心跳引擎驱动的定时自主提示和任务提醒
- **定时调度** — Cron / 固定频率任务（Agent 提示、记忆维护、数据清理、HTTP 回调）
- **渠道集成** — Webhook 端点 + ChannelAdapter SPI，支持对接微信、飞书等 IM 平台
...
后端运行在 `http://localhost:3007`，前端运行在 `http://localhost:5173`。
...
│      Chat  Agents  Skills  Models  Memory  ...      │
└──────────────────────┬──────────────────────────────┘
...
┌───────▼─────────────────────────────────────────────┐
│                intellimate-agent                     │
│  AgentRuntime → AgentLoopExecutor → ToolsEngine     │
│     ┌──────────────┐  ┌──────────────┐              │
...
┌───────▼─────────────────────────────────────────────┐
│                intellimate-memory                    │
│  WorkingMemory → Consolidation → LongTermMemory     │
