...
import { createJob } from "../../lib/schedulerApi";
...
export default function CreateJobModal({ onClose, onCreated }: CreateJobModalProps) {
  const { agents, fetchAgentList } = useAgentStore();
  ...
  const [triggerType, setTriggerType] = useState("FIXED_RATE");
  const [triggerValue, setTriggerValue] = useState("60000");
  ...
  useEffect(() => {
    if (agents.length === 0) fetchAgentList();
  }, [agents.length, fetchAgentList]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await createJob({
        ...
        jobClass: "agent-prompt",
        jobGroup: "agent",
        displayName: displayName.trim() || jobName.trim(),
        description: `Agent: ${agentName} | ${prompt.trim().substring(0, 50)}`,
        triggerType,
        triggerValue,
        timeoutMs,
        maxRetryCount,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">新建 Agent 定时任务</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-slate-600 dark:text-slate-400">任务名称 *</label>
              <input
                type="text"
                value={jobName}
          ...
          <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
            ...
            <div>
              <label className="text-sm text-slate-600 dark:text-slate-400">选择 Agent *</label>
              <select
                value={agentName}
            ...
            <textarea
              ...
              placeholder={templateMode
                ? "现在是{{timeOfDay}}，{{dayOfWeek}} {{time}}。请根据当前时间为用户提供一个适合的问候和建议。\n\n上次你说的是：{{lastResponse}}\n请不要重复上次的内容。"
                : "请检查今天的待办事项并发送提醒"
              }
            />
            ...
            {templateMode && (
              <div className="mt-2 space-y-2">
                ...
                <p className="text-xs text-slate-400 italic">
                  系统会在执行时自动替换变量为实际值（时间、上次回复等）
                </p>
              </div>
            )}

            {!templateMode && (
              <p className="text-xs text-slate-400 mt-1">到达触发时间后，系统会将此 Prompt 发送给选定的 Agent 执行</p>
            )}
          </div>
          ...
          <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
            ...
            <div className="mt-3">
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm font-mono"
                placeholder={triggerType === "CRON" ? "0 0 9 * * ?" : "60000"}
              />
