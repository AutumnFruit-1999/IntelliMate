...
**plan**（计划管理）：单一工具替代旧版的 writePlan 和 updatePlan，通过 `action` 参数分发三种操作：

| action | 参数 | 说明 |
|--------|------|------|
| `create` | `title`, `steps[]` | 创建计划。每步需 `title`、`description`、`verification`（验证方式） |
| `step_done` | `stepIndex`, `resultSummary?` | 标记步骤完成（须先通过 verification 验证） |
| `complete` | `summary?` | 完成整个计划 |

返回 JSON 格式结果，包含 `messageId`（transcript_message.id）、状态和消息。工具内置宽松的 JSON 解析逻辑（未加引号字段名、单引号、Markdown 代码块提取等）。

计划工具不通过 @Tool 注解注册，而是直接实现 ToolCallback 接口（`PlanTool`），以避免 Spring AI 对 JSON 返回值的双重编码。网关侧通过 `InlinePlanOperationsImpl` 实现 `PlanOperations` SPI。
...
计划工具（plan）和自定义工具不属于任何预定义分组。自定义工具在元数据接口中显示为 CUSTOM 分组。
