...
计划以内联消息形式展示在对话流中（`PlanMessage` 组件），不再有右侧侧边面板。
...
计划模式的轻量状态。仅跟踪当前活跃计划的指针：

- `activePlanMessageId`：活跃计划的 messageId（transcript_message.id）
- `activePlanStatus`：活跃计划的状态

计划的完整数据（步骤列表、验证条件、结果摘要等）存储在 `chatStore` 消息的 `metadata` 中。WebSocket 事件处理器通过 `handlePlanCreated`、`handleStepUpdated`、`handleStatusChanged`、`handlePlanCompleted` 更新活跃指针，同时调用 `chatStore.updateMessageMetadata` 同步消息内的 plan 数据。
...
计划相关事件包括：plan.created（新计划创建）、plan.step_updated（步骤状态变更）、plan.status_changed（计划状态变更）、plan.completed（计划执行完成）。
...
计划模式允许 Agent 创建一个分步骤的执行计划，用户可以审查和批准后让 Agent 按计划执行。计划作为内联消息展示在对话流中，不再使用侧边面板。

### PlanMessage

`PlanMessage` 是内联计划消息组件，当消息的 `metadata.type === "plan"` 时在对话流中渲染 checklist 风格的计划卡片。包含：

- 计划标题和状态徽章
- 步骤列表（状态图标、标题、描述、验证条件、结果摘要）
- 内联操作按钮：draft 状态显示「执行」「拒绝」；executing 显示「暂停」「取消」；paused 显示「继续」「取消」

步骤的修改、添加和跳过通过自然语言对话完成，不再提供拖拽排序或独立步骤编辑 UI。

### 计划生命周期

创建：Agent 调用 `plan` 工具（action=create）时，服务端发送 `plan.created` 事件。前端通过 `chatStore.addPlanMessage` 在对话流中插入计划消息，并在 `planStore` 中记录 `activePlanMessageId`。

审批：计划创建后处于 draft 状态。用户点击「执行」时，前端通过 `createPlanApprove(messageId, true)` 发送 `plan.approve` 请求；服务端审批通过后自动触发 Agent 执行。

执行：Agent 按步骤执行计划，每步完成后发送 `plan.step_updated` 事件。其他工具调用在对话气泡中正常流式展示，与 checklist 独立。

完成：收到 `plan.completed` 事件时，计划标记为已完成，`planStore` 清除活跃指针。

历史：已完成的计划作为普通消息保留在对话历史中。完整的计划历史可通过「计划历史」视图查看。

### 协议函数

`protocol.ts` 提供四个计划操作方法，均使用 `messageId` 标识：

- `createPlanApprove(messageId, approved)`
- `createPlanPause(messageId)`
- `createPlanResume(messageId)`
- `createPlanCancel(messageId)`
...
- `plan` 工具调用不显示在普通工具列表中（避免与内联 checklist 重复显示）。
...
部分组件直接使用原生 fetch 调用 API（通过 Vite 代理的相对路径），而不经过 api.ts 封装。
...
桌面端（md 断点及以上）侧边栏始终可见，计划以内联消息形式展示在对话流中。Agent 管理卡片使用 2-3 列网格布局。
