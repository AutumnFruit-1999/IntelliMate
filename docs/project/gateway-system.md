...
6. 否则进入流式 Agent 运行：
   - 检查是否有活跃的计划（executing 或 approved 状态）
   - 将用户消息持久化到 transcript_message 表（如果在执行计划中，关联 plan_id）
   - 加载对话历史（纯聊天模式下加载最近的非计划消息；计划执行模式下混合加载少量聊天历史和计划相关消息）
   - 通过 AgentConfigService.resolve 加载 Agent 的完整配置（模型、工具、MCP、技能、委派、Bridge 等）
   - 构建 AgentRunRequest，包含计划上下文和执行评估信息
   - 调用 AgentRuntime.dispatch 开始 AI 对话循环
   - 将 AgentEvent 流映射为 WebSocket EventFrame 实时发送给前端

7. 对话完成后：
   - 持久化助手的完整回复到 transcript_message
   - 执行计划后同步：如果 Agent 完成但留有未完成的计划步骤，自动标记它们为完成并可能自动完成整个计划
   - 如果计划执行完毕，可选地触发记忆提取
...
- 计划事件（PlanCreated、PlanAwaitingApproval、PlanStatusChanged、PlanStep*、PlanAdjusted、PlanCompleted）映射为 plan.* 系列事件
...
- plan.approve_and_execute：一步完成审批并开始执行
- plan.approve：审批或拒绝计划，可选地修改步骤内容
- plan.pause / plan.resume / plan.cancel：暂停、恢复和取消计划
- plan.skip_step / plan.modify_step / plan.add_step / plan.reorder_steps：步骤级别的操作
...
V12 创建了 plan 和 plan_step 表，为 transcript_message 添加了 plan_id 关联字段。
...
**PlanController**（/api/plans）：计划的列表（支持筛选）、详情、步骤和删除。
...
计划系统由四层组成：Agent 工具层（PlanOperations 接口，供 WritePlanTool 和 UpdatePlanTool 调用）、网关实现层（PlanOperationsImpl，同步包装器）、服务层（PlanService，所有计划和步骤的 CRUD 及状态机逻辑）、以及 WebSocket 和 REST 的接入层。

### 状态生命周期

计划有五种状态：draft（草稿）、approved（已审批）、executing（执行中）、paused（已暂停）、completed（已完成）和 cancelled（已取消）。状态流转为：draft → approved → executing（可在 executing 和 paused 之间切换）→ completed 或 cancelled。

步骤有五种状态：pending（待执行）、in_progress（执行中）、completed（已完成）、failed（失败）和 skipped（已跳过）。当步骤被标记为 in_progress 时，如果计划状态还是 draft 或 approved，会自动提升为 executing。步骤 failed 会触发计划自动暂停。

### 消息作用域

计划执行期间的消息通过 plan_id 字段关联到计划。加载历史时，非计划的聊天消息和计划相关消息分开查询后合并，确保 Agent 在执行计划时既能看到计划上下文也能了解之前的聊天背景。
