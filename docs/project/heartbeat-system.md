...
| ACTIVE（活跃中） | Agent 正常活跃时段 | wakeTime+1h 至 sleepTime-2h | 按间隔检查，有到期任务时主动提醒 |
...
当用户断开 WebSocket 后重新连接，`deliverPendingMessages()` 方法负责补发期间遗漏的主动消息。回放逻辑有三层保护：

1. 时间窗口：只回放 `lastDeliveredTimestamp` 之后的消息
2. TTL 过滤：回放范围不超过配置的 `message-ttl-hours`（默认 24 小时）。如果 checkpoint 早于 TTL 窗口，以 TTL 为准
3. 数量限制：单次回放不超过 `replay-limit` 条（默认 20 条）

### 去重策略

`lastDeliveredTimestamp` 按 Agent 粒度（而非 WebSocket session 粒度）记录，断开连接时不清除。这意味着同一 Agent 的多次重连不会重复收到已投递的消息。服务重启后内存丢失，但结合 TTL 过滤，重放范围受限于 TTL 窗口内。

首次绑定时不回放历史消息，只设置 checkpoint 作为后续回放的起点。
...
intellimate:
  proactive:
    message-ttl-hours: 24      # 主动消息有效期（小时），超过则不回放
    replay-limit: 20           # 重连时最大回放消息数
  scheduler:
...
消息在推送前已持久化到 `transcript_message`，因此即使用户当时不在线，重连后仍可通过回放机制收到遗漏的消息。
