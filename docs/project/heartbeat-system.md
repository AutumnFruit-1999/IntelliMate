...
当用户不在线（没有活跃的 WebSocket 连接）时，心跳系统不会为该 Agent 生成任何消息。这包括心跳问候和任务提醒——如果没有人在听，就不需要说话。

如果在用户离线期间有任务提醒到期（remind_at 已过），系统会清除该提醒的 remind_at 标记（防止下次检查重复触发），但不会生成或保存消息。提醒被视为"已过期丢弃"。

这个设计的理念是：Agent 的主动消息是实时交流，不是离线消息队列。用户回来后看到的应该是当前状态的问候（如果恰好是 WAKING 时段），而不是一堆过时的历史消息。
...
当用户断开 WebSocket 连接后，心跳系统会检测到该 Agent 无在线会话，自动跳过消息生成。到期的任务提醒会被清除（丢弃），不会累积。用户重新连接后，心跳系统恢复正常工作，按当前状态生成新的消息。
