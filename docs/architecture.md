# JavaClaw 项目分层架构与典型场景

## 一、整体鸟瞰

仓库根 [pom.xml](../pom.xml) 定义四个 **Maven 子模块**，外加仓库内的 **javaclaw-web**（独立前端工程，非 Maven module）：

| 分层 | 模块 / 目录 | 依赖关系（简述） |
|------|-------------|------------------|
| 共享内核 | [javaclaw-core](../javaclaw-core) | 无业务模块依赖；被 agent / gateway 引用 |
| 通道抽象 | [javaclaw-channel-api](../javaclaw-channel-api) | 依赖 core；定义 `ChannelAdapter` SPI |
| Agent 引擎 | [javaclaw-agent](../javaclaw-agent) | 依赖 core；Spring AI + 工具/MCP/技能运行时 |
| 可执行网关 | [javaclaw-gateway](../javaclaw-gateway) | 聚合 core、channel-api、agent；WebFlux + R2DBC + Flyway |
| 展示层 | [javaclaw-web](../javaclaw-web) | 通过 WebSocket/HTTP 调用网关 |

数据流主路径（Web 聊天）可概括为：

```mermaid
flowchart LR
  Web[javaclaw_web] -->|WS RequestFrame| GW[GatewayWebSocketHandler]
...
**整体**：跨模块共享的 **配置**（如 [JavaClawProperties](../javaclaw-core/src/main/java/com/atm/javaclaw/core/config/JavaClawProperties.java)）、**网关帧协议**（[GatewayFrame](../javaclaw-core/src/main/java/com/atm/javaclaw/core/protocol/GatewayFrame.java) / `RequestFrame` / `EventFrame` / `ResponseFrame`）、**会话键与元数据**（`SessionKey`、`SessionMetadata`）、以及统一异常类型。

**不包含**：具体 LLM 调用、数据库实体、HTTP 路由实现。

### 2. javaclaw-channel-api

**整体**：第三方 IM（微信、飞书等）的 **SPI 契约**。核心接口 [ChannelAdapter](../javaclaw-channel-api/src/main/java/com/atm/javaclaw/channel/api/ChannelAdapter.java) 将外部协议映射为 `InboundEnvelope` / `OutboundMessage`，由网关侧 [ChannelsManager](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/channel/ChannelsManager.java) 等装配；当前实现示例为 [WebChatAdapter](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/channel/webchat/WebChatAdapter.java)。

### 3. javaclaw-agent

**整体**：**与通道无关** 的 Agent 执行引擎：模型解析（`ChatModelRegistry`）、多轮对话循环（[AgentRuntime](../javaclaw-agent/src/main/java/com/atm/javaclaw/agent/runtime/AgentRuntime.java)）、工具/MCP 回调（`ToolsEngine`）、可选技能摘要注入（`SkillContentProvider`）、按会话串行排队（`RunQueueManager.enqueue`）、工具审批挂起（`ToolApprovalGate`）等。对外以 `Flux<AgentEvent>` 流式输出，由网关映射为 `EventFrame`。

### 4. javaclaw-gateway

**整体**：**唯一 Spring Boot 可运行应用**（见 [javaclaw-gateway/pom.xml](../javaclaw-gateway/pom.xml)）：WebSocket 入站、安全校验、[MessagePipeline](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/pipeline/MessagePipeline.java) 编排、会话与 transcript 持久化（R2DBC）、Agent/MCP/模型相关 Spring Bean、REST 管理接口（如 Agent/Skill 配置）等。

### 5. javaclaw-web

**整体**：React 单页应用：维护连接、发送 `conversation.message` 等请求、消费 `agent.chunk` / `agent.tool_call` 等事件并渲染 UI（[useWebSocket.ts](../javaclaw-web/src/hooks/useWebSocket.ts) 等）。

---

## 三、典型场景：按链路逐跳（WebSocket → Handler → Pipeline → SessionManager → Agent）

以下均以 **Web 通道 + WebSocket** 为主路径，与 [GatewayWebSocketHandler](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/websocket/GatewayWebSocketHandler.java)、[MessagePipeline](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/pipeline/MessagePipeline.java) 一致。帧协议见 [javaclaw-core](../javaclaw-core) 中的 `GatewayFrame` 系列类型。

### 3.0 主链路五跳总览

| 跳 | 组件 / 代码入口 | 职责摘要 |
|----|-----------------|----------|
| 1. WebSocket 传输层 | Spring `WebSocketSession` + [ProtocolCodec](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/websocket/ProtocolCodec.java) | 文本帧入站 `decode` → `GatewayFrame`；出站 `encode`。 |
| 2. GatewayWebSocketHandler | [GatewayWebSocketHandler](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/websocket/GatewayWebSocketHandler.java) | 握手后校验 token；`routeFrame` 分发；`RequestFrame` → `MessagePipeline.processRequest`；Pipeline 产出帧经 `outSink` 回写 WebSocket。 |
| 3. MessagePipeline | [MessagePipeline](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/pipeline/MessagePipeline.java) | 按 `method` 分支；`conversation.message` 拼 `SessionKey`、调 SessionManager、可选 Command 短路或进入流式对话；`mapAgentEvent` → `EventFrame` / `ResponseFrame`。 |
| 4. SessionManager | [SessionManager](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/session/SessionManager.java) / [SessionManagerImpl](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/session/SessionManagerImpl.java) | `getOrCreate` 持久化会话；`appendMessage` / `getHistory` / `resetSession` 操作 [TranscriptMessageEntity](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/entity/TranscriptMessageEntity.java)。 |
| 5. Agent | [AgentRuntime](../javaclaw-agent/src/main/java/com/atm/javaclaw/agent/runtime/AgentRuntime.java) | `dispatch` → `RunQueueManager.enqueue`（按 `sessionId` 串行）；`executeAgentLoop` 产出 `Flux<AgentEvent>`。 |
...
| **MessagePipeline** | 解析 `SessionKey` / `SessionMetadata`；`getOrCreate` 后若 **非** [CommandHandler.isCommand](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/pipeline/CommandHandler.java) 则进入 `processMessageStreaming`：`appendMessage`(user) → `getHistory` + `AgentConfigService.resolve` → 构建 `AgentRunRequest` → `agentRuntime.dispatch`；对流式 `AgentEvent` 调用 `mapAgentEvent`；流结束后 `appendMessage`(assistant) 并 `ResponseFrame.success`。 |
| **SessionManager** | `getOrCreate`：按 `channelId/contextType/contextId` 查找或新建 [SessionEntity](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/entity/SessionEntity.java)，并更新 `lastActiveAt`；`appendMessage` 写入用户与助手消息；`getHistory` 取最近 N 条并按 `createdAt` 排序。 |
...
| **MessagePipeline** | 入口即 [processApprovalResponse](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/pipeline/MessagePipeline.java)：`agentRuntime.resolveApproval(...)`，返回 `ResponseFrame.success` / `failure`。 |
...
| **SessionManager** | 另提供 [resolveSession(InboundEnvelope)](../javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/session/SessionManagerImpl.java)：`InboundEnvelope` → 同样落到 `getOrCreate`，便于 IM 适配器与 Web 共用会话模型。 |
...
[ChannelAdapter](../javaclaw-channel-api/src/main/java/com/atm/javaclaw/channel/api/ChannelAdapter.java) 负责厂商协议与 `InboundEnvelope` / `OutboundMessage` 的互转；编排责任仍在 Gateway。
