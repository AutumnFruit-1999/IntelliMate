...
钉钉支持两种接入模式，IntelliMate 均已支持：

| 模式 | 渠道 ID | 需要公网 | 适用场景 |
|------|---------|---------|---------|
| **Stream 模式**（推荐） | `dingtalk-stream` | 否 | 开发测试、内网部署、简单配置 |
| Webhook 模式 | `dingtalk` | 是 | 传统生产部署 |

### Stream 模式（推荐）

Stream 模式通过 WebSocket 长连接接收消息，无需公网 IP、域名和 HTTPS 证书。
...
1. 进入应用 → 「应用功能」→ 「机器人」
2. 开启机器人配置
3. **消息接收模式**选择「Stream 模式」
4. 无需填写回调地址

#### 3. 配置 IntelliMate

在渠道管理页面选择「钉钉 Stream」，填写：
...
配置保存并点击「连接」后，IntelliMate 会自动建立 WebSocket 长连接。在钉钉中给机器人发消息即可测试。

---

### Webhook 模式

传统 HTTP 回调方式，适合有公网 HTTPS 端点的生产环境。
...
4. 设置 **消息接收地址**：
   ...
   ```
5. 设置签名验证 — 勾选「加签」，记录 **签名密钥**

#### 3. 配置 IntelliMate

在渠道管理页面选择「钉钉 Webhook」，填写：
