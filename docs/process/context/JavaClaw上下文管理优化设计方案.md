# JavaClaw 上下文管理优化设计方案

> 版本：v1.0 | 日期：2026-04
>
> 基于《AI Agent 上下文管理主流方案调研报告》的调研结论，结合 JavaClaw 现有架构设计
...
**摘要模型选择策略**：
- 使用 `javaclaw.agent.condenser-model` 配置项指定摘要专用模型
...
##### 3.3.2 JavaClawProperties.Agent 变更

在 `JavaClawProperties.Agent` 中新增字段：
...
public class CondenserFactory {
    ...
    public ContextCondenser create(JavaClawProperties.Agent config) {
...
基于 Cursor 的公开数据，MCP 工具描述的动态发现可减少约 **47%** 的 Agent 总 token 消耗。对于 JavaClaw 中配置了大量 skills 和 MCP 工具的场景，预计可减少 System Prompt 体积 **30-50%**。
...
工具执行结果（50KB）
    ↓ 超过 file-threshold
写入 /tmp/javaclaw/{sessionId}/tool-output-{id}.txt
    ↓
上下文中注入：
...
| W1 | JavaClawProperties 新增配置项 + CondenserFactory | 配置项、工厂类 |
...
| [Plan Mode 设计](../plan/javaclaw-plan-mode-design.md) | Plan 执行历史的隔离加载逻辑需适配事件日志系统 |

---

## 八、总结

本设计方案将 JavaClaw 的上下文管理从当前的**单一截断模式**逐步演进为**多策略、可插拔、分层记忆**的架构：
