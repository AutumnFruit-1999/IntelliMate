# IntelliMate 人脑记忆架构设计方案

> 版本：v1.0 | 日期：2026-04
>
> 用人脑记忆系统的生物学模型重新设计 IntelliMate 的上下文管理架构
...
IntelliMate 当前的上下文管理机制由 **12+ 个独立的截断/压缩机制** 组成，分散在 Gateway、Agent Runtime、工具层和 Prompt 层。这些机制各自独立运作、互不感知、阈值硬编码，导致：
...
本方案提议：**用人脑记忆系统的架构模型作为 IntelliMate 上下文管理的统一设计框架**，以 3 层记忆结构 + 2 个核心过程替代现有 12+ 个碎片化机制，从根本上简化和统一架构。

---

## 二、生物学基础：人脑记忆系统

本章提供详细的神经科学背景，作为后续技术设计的生物学依据。每个概念都附有原始研究引用和与 IntelliMate 的类比预告。
...
> **IntelliMate 类比预告**：这三级结构将分别映射为工具输出直接入库（感觉寄存器）、WorkingMemory（短期/工作记忆）、LongTermMemory（长期记忆）。
...
> **IntelliMate 类比预告**：工具的原始输出就像感觉刺激——信息量巨大（一个 `exec` 命令可能返回数万字符），完整进入工作记忆。当工作记忆接近容量上限时，记忆巩固机制会智能压缩旧内容，类似大脑的注意力选择——不是在输入端丢弃信息，而是在处理过程中决定哪些需要详细保留、哪些可以摘要化。
...
> **IntelliMate 类比预告**：WorkingMemory 的 tokenBudget 对应工作记忆容量；MemoryChunk 的 chunking 策略直接借鉴 Miller 的分块理论（一组 tool_call + tool_response = 1 个 chunk）；注意力权重（relevance scoring）对应 Cowan 的注意力焦点选择机制。
...
> **IntelliMate 类比预告**：LongTermMemory 的三个子系统直接映射自这个分类体系——EpisodicMemory（会话经历记录）、SemanticMemory（项目知识和用户偏好）、ProceduralMemory（成功的工具使用模式和工作流模板）。
...
> **IntelliMate 类比预告**：MemoryConsolidator 直接模拟记忆巩固过程——当工作记忆（短期）接近容量上限时，用 LLM 对旧内容做"回放式摘要"（类似睡眠回放），提取要旨（保留核心信息）和语义事实（写入长期记忆），替代现有的 substring 盲截断。
...
> **IntelliMate 类比预告**：MemoryRetrieval 直接实现编码特异性原则——用当前用户消息和任务上下文作为"检索线索"，从长期记忆中找到**编码时上下文最匹配的记忆**。access_count 的递增机制对应"检索练习效应"——越常被检索的记忆越稳固。
...
> **IntelliMate 类比预告**：遗忘机制是长期记忆的"垃圾回收"——通过 `importance × recency_decay × access_frequency` 的衰减公式模拟艾宾浩斯遗忘曲线，主动清理不再相关的记忆，保持记忆库的"信噪比"。
...
| 生物学概念 | 核心研究者 | 关键参数 | IntelliMate 映射 |
|-----------|-----------|---------|-------------|
...
## 三、问题诊断：IntelliMate 现有上下文管理为何复杂

### 3.1 现有机制清单（12+个独立组件）

当前 IntelliMate 的上下文管理由以下独立机制组成，各自有独立的配置、阈值和代码实现：
...
## 四、独立模块设计：intellimate-memory

### 4.1 模块定位

基于人脑记忆模型，创建一个**完全独立的 Maven 模块 `intellimate-memory`**，从 `intellimate-agent` 和 `intellimate-gateway` 中剥离所有上下文管理逻辑，统一收归记忆模块。
...
```mermaid
flowchart TD
    Core["intellimate-core"]
    ChannelApi["intellimate-channel-api"]
    Agent["intellimate-agent"]
    Gateway["intellimate-gateway"]
...
**改造后**：新增独立的 `intellimate-memory` 模块

```mermaid
flowchart TD
    Core["intellimate-core"]
    Memory["intellimate-memory NEW"]
    ChannelApi["intellimate-channel-api"]
    Agent["intellimate-agent"]
    Gateway["intellimate-gateway"]
...
| 模块 | 改造前职责 | 改造后职责 | 迁出内容 |
|------|----------|----------|---------|
| **intellimate-memory** | 不存在 | 全部记忆管理: 工作记忆、巩固、长期记忆、检索、遗忘 | — |
| **intellimate-agent** | 包含 ContextWindowTracker / ContextCondenser / truncateToolResult / 各工具截断 | 专注于 Agent 执行循环和工具调用，通过 `intellimate-memory` API 管理上下文 | ContextWindowTracker, ContextCondenser, truncateToolResult |
| **intellimate-gateway** | 包含 historyLimit 查询 / 消息转换 | 专注于协议处理和消息路由，通过 `intellimate-memory` API 加载和存储记忆 | historyLimit 逻辑迁入 MemoryRetrieval |
| **intellimate-core** | 配置和公共模型 | 新增 MemoryChunk 等公共模型（或放在 memory 模块内） | — |

#### 4.1.3 intellimate-memory 模块结构

```
intellimate-memory/
├── pom.xml
└── src/main/java/com/atm/intellimate/memory/
...
```xml
<artifactId>intellimate-memory</artifactId>
<name>IntelliMate Memory</name>
<description>人脑记忆架构的上下文管理模块</description>

<dependencies>
    <dependency>
        <groupId>com.atm</groupId>
        <artifactId>intellimate-core</artifactId>
    </dependency>
...
`MemorySystem` 是 `intellimate-memory` 对外暴露的**唯一入口**，`intellimate-agent` 和 `intellimate-gateway` 仅通过此接口与记忆系统交互：
...
| 原文件 | 原模块 | 迁移到 | 说明 |
|-------|-------|-------|------|
| `ContextWindowTracker.java` | intellimate-agent | `intellimate-memory` WorkingMemory 内部 | token 追踪逻辑合并为 WorkingMemory 的内部方法 |
| `ContextCondenser.java` | intellimate-agent | 废弃，不迁移 | 新架构不使用截断，完全依赖 LLM 巩固 |
| `AgentRuntime.truncateToolResult()` | intellimate-agent | 废弃，不迁移 | 工具输出完整进入工作记忆，无需截断 |
| 各工具的截断常量 | intellimate-agent | 废弃，不迁移 | ExecTool/FileReadTool 等不再自行截断 |
| `MessagePipeline.loadHistory()` | intellimate-gateway | `intellimate-memory` MemoryRetrieval | 历史加载逻辑迁入检索模块 |
| `TOTAL_MAX_CHARS` 截断 | intellimate-agent | `intellimate-memory` WorkingMemory | 合并为 tokenBudget 统一约束 |

### 4.3 改造后的调用关系

**intellimate-agent 中的 AgentRuntime（改造后）**：
...
**intellimate-gateway 中的 MessagePipeline（改造后）**：
...
tokens:     8000
content:    "你是 IntelliMate 智能编码助手...（完整系统提示词）"
...
content:    "[长期记忆] 该项目使用 Spring Boot 3.4.3 + R2DBC + WebFlux，
             AuthService 位于 intellimate-gateway/src/main/java/.../AuthService.java"
...
importance: 0.8
tokens:     9150
content:    [Assistant 调用 exec("mvn test -pl intellimate-gateway")]
            +
...
Plan 模式是 IntelliMate 的多步骤任务执行模式，Agent 按计划分步执行。Plan 模式下记忆流转有以下特殊之处：
...
```yaml
# 分散在不同层级的 12+ 个配置/常量
intellimate.agent.history-limit: 50
intellimate.agent.max-context-tokens: 128000
intellimate.agent.max-tool-result-chars: 16000
intellimate.agent.max-turns: 128
intellimate.agent.condenser-keep-recent: 20
intellimate.agent.condenser-summary-length: 200
intellimate.agent.condenser-min-turns-between: 5
# 硬编码常量（未暴露为配置）
...
创建独立的 `intellimate-memory` Maven 模块，建立基础架构。

| 任务 | 交付物 |
|------|-------|
| 创建 `intellimate-memory` 模块目录和 pom.xml | Maven 模块骨架 |
| 在父 POM 的 `<modules>` 中注册 | 构建集成 |
| 在 `intellimate-agent` 和 `intellimate-gateway` 中添加依赖 | 依赖链建立 |
...
| **Flyway 迁移脚本位置** | agent_memory 表的 migration 放在哪个模块 | Flyway 脚本保留在 intellimate-gateway（主应用），memory 模块仅定义 Entity |
...
| [IntelliMate上下文管理优化设计方案](IntelliMate上下文管理优化设计方案.md) | 本方案的替代方案；如采纳本方案则该文档归档为历史参考 |
