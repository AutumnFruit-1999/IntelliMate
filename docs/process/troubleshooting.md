...
[.../com/atm/javaclaw/gateway/http/HealthEndpoint.class] and overriding is disabled.
```
...
**涉及文件**：
- 删除 `javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/http/HealthEndpoint.java`
- 修改 `javaclaw-gateway/src/main/resources/application.yml`
...
**根因**：  
Spring AI Alibaba 1.0.0.2 的 DashScope 自动配置类（Agent、Embedding、Image、Rerank、AudioSpeech、AudioTranscription）内部都依赖 `RestClient.Builder`，这是 Spring MVC（Servlet 栈）提供的组件。JavaClaw 使用 Spring WebFlux（Reactive 栈），只有 `WebClient.Builder`，没有 `RestClient.Builder`。

JavaClaw 只使用 `ChatClient` 进行 LLM 对话，其他能力（向量嵌入、图像生成、语音等）暂不需要。
...
**涉及文件**：
- 修改 `javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/JavaClawApplication.java`
...
spring:
  flyway:
    enabled: true
    url: jdbc:mysql://${MYSQL_HOST:localhost}:${MYSQL_PORT:3306}/${MYSQL_DATABASE:javaclaw}?useUnicode=true&characterEncoding=utf-8&useSSL=false&allowPublicKeyRetrieval=true
...
**涉及文件**：
- 修改 `javaclaw-gateway/src/main/resources/application.yml` — 移除 `spring.datasource.*`，改用 `spring.flyway.url/user/password`
- 修改 `javaclaw-gateway/pom.xml` — 移除 `spring-boot-starter-jdbc`
...
at com.atm.javaclaw.agent.runtime.AgentRuntime.executeRun
```
...
**涉及文件**：
- 修改 `javaclaw-agent/src/main/java/com/atm/javaclaw/agent/runtime/AgentRuntime.java`
...
**根因**：  
`AgentController.getAgent()` 直接查询 `agent` 表，找不到记录就抛 404。但 JavaClaw 的设计允许只通过 `application.yml` 配置默认 agent 而不在 DB 中创建记录 —— `AgentConfigService.resolve()` 已经实现了这种回退逻辑，`AgentController` 却没有。
...
**涉及文件**：
- 修改 `javaclaw-gateway/src/main/java/com/atm/javaclaw/gateway/http/AgentController.java`
...
JavaClaw 使用纯响应式栈（WebFlux + R2DBC），如果引入的第三方依赖自动配置了 Servlet 栈组件（如 `RestClient.Builder`、`RestTemplate`、`HttpServletRequest`），需要通过 `@SpringBootApplication(exclude = ...)` 排除。
