...
JavaClaw 当前的 Agent 执行是一个标准 ReAct 循环（`AgentRuntime.executeLoopTurn`），对用户而言是黑盒：
...
```mermaid
graph TB
    subgraph frontend [Frontend - javaclaw-web]
        UI[Chat UI]
        PlanView[Plan View 组件]
        PlanStore[planStore]
    end

    subgraph gateway [Gateway - javaclaw-gateway]
        MP[MessagePipeline]
        PS[PlanService]
    ...
    subgraph agent [Agent - javaclaw-agent]
        AR[AgentRuntime]
        WPT[WritePlanTool]
        UPT[UpdatePlanTool]
        TE[ToolsEngine]
...
| 模块 | 层 | 职责 |
|------|---|------|
| `PlanService` | javaclaw-gateway | Plan 状态管理（CRUD、状态流转）——不编排执行 |
| `WritePlanTool` | javaclaw-agent | Agent 可调用的工具，创建结构化计划 |
| `UpdatePlanTool` | javaclaw-agent | Agent 可调用的工具，标记/增删步骤/提前完成 |
| `PlanEntity` | javaclaw-gateway | 计划数据模型 |
| `PlanStepEntity` | javaclaw-gateway | 计划步骤数据模型 |
| `PlanRepository` | javaclaw-gateway | 计划持久化 |
| `PlanStepRepository` | javaclaw-gateway | 步骤持久化 |
| `planStore` | javaclaw-web | 前端 Plan 状态管理 |
| `PlanView` | javaclaw-web | 前端 Plan 可视化组件 |
...
| `JavaClawProperties.java` | 扩展 | 新增 3 个 Plan 配置字段 |
...
| 文件 | 层 | 职责 |
|------|---|------|
| `WritePlanTool.java` | javaclaw-agent/tools | 创建计划的工具 |
| `UpdatePlanTool.java` | javaclaw-agent/tools | 更新计划（标记/增删/完成）的工具 |
| `PlanService.java` | javaclaw-gateway/service | Plan 状态管理服务 |
| `PlanEntity.java` | javaclaw-gateway/entity | 计划实体 |
| `PlanStepEntity.java` | javaclaw-gateway/entity | 步骤实体 |
| `PlanRepository.java` | javaclaw-gateway/repository | 计划 DAO |
| `PlanStepRepository.java` | javaclaw-gateway/repository | 步骤 DAO |
...
| `planStore.ts` | javaclaw-web/src/stores | 前端状态管理 |
| `PlanView.tsx` | javaclaw-web/src/components | 计划视图组件 |
| `PlanStepCard.tsx` | javaclaw-web/src/components | 步骤卡片组件 |
