# JavaClaw 工具管理：现状分析与改造方案

> 本文档分析 JavaClaw 当前工具管理现状，对标 OpenClaw 的能力，提出改造方案。
...
```java
// javaclaw-agent/src/main/java/com/atm/javaclaw/agent/tools/ExecTool.java
...
```java
// javaclaw-agent/src/main/java/com/atm/javaclaw/agent/tools/ToolAutoConfiguration.java
@Bean
...
```java
// javaclaw-agent/src/main/java/com/atm/javaclaw/agent/tools/ToolsEngine.java
...
```java
// javaclaw-agent/src/main/java/com/atm/javaclaw/agent/tools/ToolProfile.java
...
```java
// javaclaw-agent/src/main/java/com/atm/javaclaw/agent/runtime/AgentRuntime.java
...
```java
// javaclaw-agent/src/main/java/com/atm/javaclaw/agent/tools/ToolGroup.java
...
- 在 `javaclaw-gateway` 新增 `ToolController`（或扩展 `AgentController`）
...
对标 OpenClaw 的 Config 表单（Schema 驱动），JavaClaw 采用更直观的**工具列表 + Profile 选择器**方式（因为工具数量少，无需 Schema 驱动的通用方案）。
...
| 设计点 | OpenClaw | JavaClaw | 原因 |
|--------|----------|----------|------|
| UI 方式 | Schema 驱动的通用表单 | 专用的工具列表 + 分组 | 工具少（6个），专用 UI 体验更好 |
| 存储方式 | 本地 JSON5 文件 | MySQL JSON 字段 | JavaClaw 是 Web 服务，需要 DB 持久化 |
| 分组定义 | 框架层硬编码映射 | Java 枚举 | 同为静态映射，Java 枚举更类型安全 |
| 通信方式 | WebSocket 直连 Gateway | REST API | JavaClaw 已有成熟的 REST API 体系 |
| byProvider | 支持 | 暂不支持 | JavaClaw 当前只用 DashScope，无需按 provider 区分 |
